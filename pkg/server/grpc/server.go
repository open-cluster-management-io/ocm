		...
		NewControllerCommandConfig(
			...
			func(ctx context.Context, controllerContext *controllercmd.ControllerContext) error {
				...
				return grpcoptions.NewServer(grpcServerOpts).WithPreStartHooks(clients).WithAuthenticator(
					...
					services.NewClusterService(clients.clusterClient, clients.clusterInformers.Cluster().V1().ManagedClusters()),
				).WithService(
					csr.CSREventDataType,
					services.NewCSRService(clients.kubeClient, clients.kubeInformers.Certificates().V1().CertificateSigningRequests()),
				).WithService(
					addonce.ManagedClusterAddOnEventDataType,
					services.NewAddonService(clients.addonClient, clients.addonInformers.Addon().V1alpha1().ManagedClusterAddOns()),
				).WithService(
					eventce.EventEventDataType,
					services.NewEventService(clients.kubeClient),
				).WithService(
					leasece.LeaseEventDataType,
					services.NewLeaseService(clients.kubeClient, clients.kubeInformers.Coordination().V1().Leases()),
				).WithService(
					payload.ManifestBundleEventDataType,
					services.NewWorkService(clients.workClient, clients.workInformers.Work().V1().ManifestWorks()),
				).Run(ctx)
			},
			clock.RealClock{},
		)
