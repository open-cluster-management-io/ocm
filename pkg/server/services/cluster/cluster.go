...
func (c *ClusterService) Get(_ context.Context, resourceID string) (*cloudevents.Event, error) {
	...
	return c.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: clusterce.ManagedClusterEventDataType}, cluster)
}
...
func (c *ClusterService) List(listOpts types.ListOptions) ([]*cloudevents.Event, error) {
	...
	evt, err := c.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: clusterce.ManagedClusterEventDataType}, cluster)
	if err != nil {
		return nil, err
	}
