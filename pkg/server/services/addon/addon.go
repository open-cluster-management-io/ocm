...
func (s *AddonService) Get(_ context.Context, resourceID string) (*cloudevents.Event, error) {
	...
	return s.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: addonce.ManagedClusterAddOnEventDataType}, addon)
}
...
func (s *AddonService) List(listOpts types.ListOptions) ([]*cloudevents.Event, error) {
	...
	for _, addon := range addons {
		evt, err := s.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: addonce.ManagedClusterAddOnEventDataType}, addon)
		if err != nil {
			return nil, err
		}
