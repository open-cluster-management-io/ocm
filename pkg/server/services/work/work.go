...
func (w *WorkService) Get(ctx context.Context, resourceID string) (*cloudevents.Event, error) {
	...
	return w.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: payload.ManifestBundleEventDataType}, work)
}
...
func (w *WorkService) List(listOpts types.ListOptions) ([]*cloudevents.Event, error) {
	...
	for _, work := range works {
		evt, err := w.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: payload.ManifestBundleEventDataType}, work)
		if err != nil {
			return nil, err
		}
