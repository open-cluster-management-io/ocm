...
func (c *CSRService) Get(_ context.Context, resourceID string) (*cloudevents.Event, error) {
	...
	return c.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: csrce.CSREventDataType}, csr)
}
...
func (c *CSRService) List(listOpts types.ListOptions) ([]*cloudevents.Event, error) {
	...
	for _, csr := range csrs {
		evt, err := c.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: csrce.CSREventDataType}, csr)
		if err != nil {
			return nil, err
		}
