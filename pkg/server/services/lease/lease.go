...
func (l *LeaseService) Get(ctx context.Context, resourceID string) (*cloudevents.Event, error) {
	...
	return l.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: leasece.LeaseEventDataType}, lease)
}
...
func (l *LeaseService) List(listOpts types.ListOptions) ([]*cloudevents.Event, error) {
	...
	for _, lease := range leases {
		cloudevt, err := l.codec.Encode(helpers.CloudEventsKubeSource, types.CloudEventsType{CloudEventsDataType: leasece.LeaseEventDataType}, lease)
		if err != nil {
			return nil, err
		}
