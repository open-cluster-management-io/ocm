...
func TestAddonAnnotationsFromManagedCluster_hostingClusterNamePrecedence(t *testing.T) {
	cases := []struct {
		name     string
		cluster  *clusterv1.ManagedCluster
		expected string
		...
		{
			name: "import-derived value overrides copied addon hosting-cluster-name",
			cluster: &clusterv1.ManagedCluster{
				ObjectMeta: metav1.ObjectMeta{
					Name: "cluster1",
					Annotations: map[string]string{
						...
						AnnotationKlusterletHostingClusterName:       "local-cluster",
						AnnotationEnableHostedModeAddons:             "true",
						addonv1beta1.HostingClusterNameAnnotationKey: "stale-addon-value",
					},
				},
			},
			expected: "local-cluster",
		...
		{
			name: "copied addon hosting-cluster-name kept when derivation does not apply",
			cluster: &clusterv1.ManagedCluster{
				ObjectMeta: metav1.ObjectMeta{
					Name: "cluster1",
					Annotations: map[string]string{
			...
			expected: "hosting-cluster",
		},
	}

	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := AddonAnnotationsFromManagedCluster(c.cluster)[addonv1beta1.HostingClusterNameAnnotationKey]
			if got != c.expected {
				t.Fatalf("expected hosting cluster name %q, got %q", c.expected, got)
			}
		})
	}
}
