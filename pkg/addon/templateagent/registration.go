		...
		for _, registration := range template.Spec.Registration {
			...
			case addonapiv1alpha1.RegistrationTypeKubeClient:
				if !contain(registrationConfigs, certificatesv1.KubeAPIServerClientSignerName) {
					configs, err := kubeClientSignerConfigurations(a.addonName, a.agentName)(cluster, addon)
					if err != nil {
...
func kubeClientSignerConfigurations(addonName, agentName string) agent.CSRConfigurationsFunc {
