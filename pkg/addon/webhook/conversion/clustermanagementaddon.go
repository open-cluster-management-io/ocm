...
const (
	// PlaceholderConfigName is used when converting v1alpha1 ConfigMeta without defaultConfig to v1beta1
	PlaceholderConfigName = "PLACEHOLDER"
)
...
func ConvertConfigMetaToAddOnConfig(configMetas []addonv1alpha1.ConfigMeta) []addonv1beta1.AddOnConfig {
	...
	for _, configMeta := range configMetas {
		...
		// If there's a default config, use it; otherwise use PLACEHOLDER
		if configMeta.DefaultConfig != nil {
			...
			// Use PLACEHOLDER as name when there's no default config
			addOnConfig.ConfigReferent = addonv1beta1.ConfigReferent{
				Name: PlaceholderConfigName,
			}
		}
...
func ConvertAddOnConfigToConfigMeta(addOnConfigs []addonv1beta1.AddOnConfig) []addonv1alpha1.ConfigMeta {
	...
	for _, addOnConfig := range addOnConfigs {
		...
		// If there's a config referent and it's not PLACEHOLDER, convert it to default config
		// PLACEHOLDER indicates there was no defaultConfig in the original v1alpha1
		if addOnConfig.Name != "" && addOnConfig.Name != PlaceholderConfigName {
