...
func TestGet_IncompleteResourceMeta(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expectedMsg: "Resource meta is incompleted",
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if condition.Reason != "IncompletedResourceMeta" {
				t.Errorf("Expected reason IncompletedResourceMeta, got %s", condition.Reason)
			}
