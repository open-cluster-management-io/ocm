...
func (f *fakeDriver) IsHubKubeConfigValid(ctx context.Context, secretOption SecretOption) (bool, error) {
	return true, nil
}

func (f *fakeDriver) BuildKubeConfigFromBootstrap(config *clientcmdapi.Config) (*clientcmdapi.Config, error) {
	return config, nil
}
...
func (f *fakeDriver) Process(
	...
	_ *corev1.Secret,
	_ map[string][]byte,
	_ events.Recorder, opt any) (*corev1.Secret, *metav1.Condition, error) {
