...
// TestSyncDeployWithoutCaBundle tests that sync returns an error when the CA bundle ConfigMap
...
func TestSyncDeployWithoutCaBundle(t *testing.T) {
	...
	err := tc.clusterManagerController.sync(ctx, syncContext, "testhub")
	if err == nil {
		t.Fatal("Expected error when CA bundle ConfigMap is missing, but got nil")
	}

	// Verify the error message indicates we're waiting for the CA bundle
	expectedErrSubstring := "CA bundle configmap"
