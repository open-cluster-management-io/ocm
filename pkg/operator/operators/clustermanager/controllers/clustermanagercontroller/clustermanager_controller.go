...
func (n *clusterManagerController) sync(ctx context.Context, controllerContext factory.SyncContext) error {
	...
	config.ManagedClusterIdentityCreatorRole, config.AwsResourceTags = getIdentityCreatorRoleAndTags(*clusterManager)
...
func getIdentityCreatorRoleAndTags(cm operatorapiv1.ClusterManager) (string, []string) {
	if cm.Spec.RegistrationConfiguration != nil {
		for _, registrationDriver := range cm.Spec.RegistrationConfiguration.RegistrationDrivers {
			if registrationDriver.AuthType == "awsirsa" && registrationDriver.AwsIrsa != nil {
				...
				return "arn:aws:iam::" + hubClusterAccountId + ":role/" + hubClusterName + "_managed-cluster-identity-creator", registrationDriver.AwsIrsa.Tags
			}
		}
	}
	return "", nil
