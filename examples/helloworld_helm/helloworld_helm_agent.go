...
// HelloWorldHelm Agent is an example that syncs configmap in cluster namespace of hub cluster
// to the install namespace in managedcluster with the suffix -<managedCluster name>.

func newAgentCommand() *cobra.Command {
	o := NewAgentOptions()
	cmd := controllercmd.
		NewControllerCommandConfig("helloworldhelm-addon-agent", version.Get(), o.RunAgent).
		NewCommand()
	cmd.Use = "agent"
...
func newAgentController(
	...
	return factory.New().WithInformersQueueKeyFunc(
		...
		WithSync(c.sync).ToController("helloworldhelm-agent-controller", recorder)
}
...
func (c *agentController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	configmap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-%s", cm.Name, clusterName),
