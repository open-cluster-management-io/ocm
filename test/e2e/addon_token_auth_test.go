...
var _ = ginkgo.Describe("Template addon with token-based authentication", ginkgo.Ordered, ginkgo.Label("addon-token-auth"), func() {
	...
	ginkgo.It("Should work with token-based authentication flow", func() {
		ginkgo.By("Step 1: Update klusterlet to use token-based authentication for addons")
		...
		ginkgo.By("Step 2: Create addon template resources")
		...
		ginkgo.By("Step 3: Create the template addon")
		...
		ginkgo.By("Step 4: Wait for addon to become available with token authentication")
		gomega.Eventually(func() error {
			return hub.CheckManagedClusterAddOnStatus(universalClusterName, addOnName)
		}, "5m", "10s").Should(gomega.Succeed())

		ginkgo.By("Step 5: Verify hub kubeconfig secret is created with token authentication")
		gomega.Eventually(func() error {
		...
		ginkgo.By("Step 6: Verify custom client cert secret is created")
		gomega.Eventually(func() error {
		...
		ginkgo.By("Step 7: Test addon functionality - create configmap on hub")
		...
		ginkgo.By("Step 8: Verify addon copies configmap to spoke using token auth")
		gomega.Eventually(func() error {
		...
		ginkgo.By("Step 9: Switch klusterlet back to CSR-based authentication")
		gomega.Eventually(func() error {
			klusterlet, err := spoke.OperatorClient.OperatorV1().Klusterlets().Get(
				context.TODO(), universalKlusterletName, metav1.GetOptions{})
			if err != nil {
		...
		ginkgo.By("Verify klusterlet is updated back to CSR auth")
		gomega.Eventually(func() error {
			klusterlet, err := spoke.OperatorClient.OperatorV1().Klusterlets().Get(
				context.TODO(), universalKlusterletName, metav1.GetOptions{})
			if err != nil {
			...
			if klusterlet.Spec.RegistrationConfiguration == nil ||
				klusterlet.Spec.RegistrationConfiguration.AddOnKubeClientRegistrationDriver == nil {
				return fmt.Errorf("CSR auth configuration not set")
			}

			if klusterlet.Spec.RegistrationConfiguration.AddOnKubeClientRegistrationDriver.AuthType != "csr" {
				return fmt.Errorf("auth type is not csr: %s",
					klusterlet.Spec.RegistrationConfiguration.AddOnKubeClientRegistrationDriver.AuthType)
			}
		...
		ginkgo.By("Step 10: Wait for addon to be available again with CSR auth")
		// After switching back to CSR, addon should re-register and become available
		// CSRs will be auto-approved by the system
		gomega.Eventually(func() error {
			return hub.CheckManagedClusterAddOnStatus(universalClusterName, addOnName)
		}, "5m", "10s").Should(gomega.Succeed())

		ginkgo.By("Step 11: Verify hub kubeconfig secret now uses CSR-based authentication")
		gomega.Eventually(func() error {
			secret, err := hub.KubeClient.CoreV1().Secrets(addonInstallNamespace).Get(context.TODO(),
				templateagent.HubKubeconfigSecretName(addOnName), metav1.GetOptions{})
			if err != nil {
			...
			// Verify the secret contains CSR-based kubeconfig (with certificate)
			...
			// CSR-based kubeconfig should contain client-certificate-data
			kubeconfigStr := string(kubeconfig)
			if !strings.Contains(kubeconfigStr, "client-certificate-data:") {
				return fmt.Errorf("kubeconfig should contain client-certificate-data when using CSR auth, got: %s", kubeconfigStr)
			}

			// Should NOT contain standalone token field (token auth)
			if _, ok := secret.Data["token"]; ok {
				return fmt.Errorf("secret should not contain token field when using CSR auth")
			}

			return nil
		}).Should(gomega.Succeed())

		ginkgo.By("Step 12: Test addon functionality after switching back to CSR")
		configmap2 := &corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				Name:      fmt.Sprintf("config-csr-%s", rand.String(6)),
				Namespace: universalClusterName,
			},
			Data: map[string]string{
				"key1": rand.String(6),
		...
		_, err = hub.KubeClient.CoreV1().ConfigMaps(universalClusterName).Create(
			context.Background(), configmap2, metav1.CreateOptions{})
		gomega.Expect(err).ToNot(gomega.HaveOccurred())

		ginkgo.By("Verify addon copies second configmap using CSR auth")
		gomega.Eventually(func() error {
			copiedConfig, err := spoke.KubeClient.CoreV1().ConfigMaps(addonInstallNamespace).Get(
				context.Background(), configmap2.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}

			if !equality.Semantic.DeepEqual(copiedConfig.Data, configmap2.Data) {
				return fmt.Errorf("expected configmap is not correct, %v", copiedConfig.Data)
			}
			return nil
		}, "2m", "5s").ShouldNot(gomega.HaveOccurred())

		ginkgo.By("Step 13: Cleanup - Delete the addon")
		...
		ginkgo.By("Step 14: Delete addon template resources")
		...
		ginkgo.By("Step 15: Cleanup CSRs")
		gomega.Eventually(func() error {
